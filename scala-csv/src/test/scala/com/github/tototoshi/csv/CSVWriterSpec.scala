...
class CSVWriterSpec extends AnyFunSpec with Matchers with BeforeAndAfter with Using {
  ...
  describe("CSVWriter") {
    ...
    describe("#writeNext") {
      ...
      describe("When a field contains quoteChar in it and escapeChar overwritten with customFormat") {
        it("should escape the quoteChar") {
          using(CSVWriter.open(new FileWriter("test.csv"))(CustomFormat)) { writer =>
            writer.writeRow(List("a", "b\"", "c"))
            writer.writeRow(List("d", "e", "f"))
          }
